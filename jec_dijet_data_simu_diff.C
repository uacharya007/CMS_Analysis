...
void jec_dijet_data_simu_diff(const char *alphaCond="015", double alphaCd=0.15){


  gROOT->Reset();

 
  // TFile *fIn1= new TFile("/afs/cern.ch/user/u/uacharya/eos/cms_analaysis/JEC_analysis/miniAOD_output/dijet_trigger_response_vs_eta7_alphaCond.root");

  //TFile *fIn2= new TFile("/afs/cern.ch/user/u/uacharya/eos/cms_analaysis/JEC_analysis/histo_eff_JEC/dijet_balance_after_JEC_simu.root");
...
//TFile *fIn= new TFile("jec_dijet_balance_without_JEC.root");
  ...
  TH1F *hAssym1[npt][22];
  TH1F *hAss_nent1[npt];
  float meanAssym1[npt][22];
  float dmeanAssym1[npt][22];
  float response1[npt][22];
  float dresponse1[npt][22];
  TH1F *hResponse1[npt];
  float nEntries_data[npt][22];
  TH1F *hAssym2[npt][22];
  float meanAssym2[npt][22];
  float dmeanAssym2[npt][22];

  float dstdDev1[npt][22];
  float response2[npt][22];
  float dresponse2[npt][22];
  TH1F *hResponse2[npt];
  float dstdDev2[npt][22];
  float nEntries_sim[npt][22];
  TH1F *hAss_nent2[npt];
  ...
  TLegend *leg= new TLegend(0.3,0.1,0.65,0.4);
  leg->AddEntry(hResponse2[0]," 50.0 < p_{T}^{avg}(GeV/C) < 90.0 ","pl");
  leg->AddEntry(hResponse2[1]," 90.0 < p_{T}^{avg}(GeV/C) < 120.0 ","pl");
  leg->AddEntry(hResponse2[2]," 120.0 < p_{T}^{avg}(GeV/C) < 160.0 ","pl");
  leg->AddEntry(hResponse2[3]," 160.0 < p_{T}^{avg}(GeV/C) < 200.0 ","pl");
  leg->AddEntry(hResponse2[4]," 200.0 < p_{T}^{avg}(GeV/C) < 500.0 ","pl");
  leg->Draw();
  // cR2->SaveAs(Form("histPlots_noAsymCut/Response_@_\u03B1_%s.pdf",alphaCond),"RECREATE");
  ...
  TLegend *leg2= new TLegend(0.35,0.7,0.75,0.88);
  leg2->AddEntry(hDataSimDiff[0]," 50.0 < p_{T}^{avg}(GeV/C) < 90.0 ","pl");
  leg2->AddEntry(hDataSimDiff[1]," 90.0 < p_{T}^{avg}(GeV/C) < 120.0 ","pl");
  leg2->AddEntry(hDataSimDiff[2]," 120.0 < p_{T}^{avg}(GeV/C) < 160.0 ","pl");
  leg2->AddEntry(hDataSimDiff[3]," 160.0 < p_{T}^{avg}(GeV/C) < 200.0 ","pl");
  leg2->AddEntry(hDataSimDiff[4]," 200.0 < p_{T}^{avg}(GeV/C) < 500.0 ","pl");
  leg2->Draw();
  // cR3->SaveAs(Form("histPlots_noAsymCut/MC_Data_Ratio_@_\u03B1_%s.pdf",alphaCond),"RECREATE");
  //leg2->Draw();
  ...
  TLegend *leg4= new TLegend(0.65,0.7,0.9,0.88);
  leg4->AddEntry(hAss_nent2[0]," 50.0 < p_{T}^{avg}(GeV/C) < 90.0 ","pl");
  leg4->AddEntry(hAss_nent2[1]," 90.0 < p_{T}^{avg}(GeV/C) < 120.0 ","pl");
  leg4->AddEntry(hAss_nent2[2]," 120.0 < p_{T}^{avg}(GeV/C) < 160.0 ","pl");
  leg4->AddEntry(hAss_nent2[3]," 160.0 < p_{T}^{avg}(GeV/C) < 200.0 ","pl");
  leg4->AddEntry(hAss_nent2[4]," 200.0 < p_{T}^{avg}(GeV/C) < 500.0 ","pl");
  leg4->Draw();
  //cR4->SaveAs(Form("histPlots/NEvents_in_Asymmetry_plot_per_@_\u03B1_Vs_\u03B7_%s.pdf",alphaCond),"RECREATE");

  /* TCanvas *cR5= new TCanvas("cR5","cR5",15,10,800,800); */
  /* hDataSimDiff_nEntries[0]->Draw(); */
  /* hDataSimDiff_nEntries[1]->Draw("same"); */
  /* hDataSimDiff_nEntries[2]->Draw("same"); */
  /* hDataSimDiff_nEntries[3]->Draw("same"); */
  /* hDataSimDiff_nEntries[4]->Draw("same"); */
  /* gPad->SetGridy(); */
  /* gPad->SetGridx(); */
  /* gPad->SetLeftMargin(0.12); // Moves the plot to the right */
  /* leg4->Draw(); */
...
#if 1
  TFile *outputFile = new TFile(Form("alphaCond/dijet_balance_alphaCond_%s_newMethod.root", alphaCond), "RECREATE");
