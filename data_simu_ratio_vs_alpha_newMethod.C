...
void data_simu_ratio_vs_alpha_newMethod(){
  gROOT->Reset();
  const int npt= 5;
  gStyle->SetOptStat(0);

  ofstream outFile("L2Residual_lowPU_pp13TeV_newMethod.txt");
  if (!outFile.is_open()) {
  ...
  const int totBins=5;
  float Xval[]={0.10,0.15,0.20,0.25,0.30};
  ...
  for(int ita=0;ita<nEta;ita++){
    ...
    multiGraph_vs_alpha_for_alpha30[ita]->SetTitle(Form("[%.1f < #eta < %.1f];#alpha;MC/Data", etaBin[ita],etaBin[ita+1]));
  ...
  for( int ipt=0; ipt<npt;ipt++){
    for(int ita=0;ita<nEta;ita++){
      ...
      c_factor[ita][ipt]=datSim_ratio_per_alphaVal_perpt_perEtaval[4][ipt][ita]-(kFSRVal[ita]*datSim_ratio_per_alphaVal_perpt_perEtaval[4][ipt][ita]*0.3);
      dc_factor[ita][ipt]=c_factor[ita][ipt]*sqrt(pow(ddatSim_ratio_per_alphaVal_perpt_perEtaval[4][ipt][ita]/datSim_ratio_per_alphaVal_perpt_perEtaval[4][ipt][ita],2));
 ...
 float  ddat_sim_ratio_alpha0_perEta_Vs_pT_newMethod[npt][nEta];
  ...
  for( int ipt=0; ipt<npt;ipt++){
      ...
      if(ipt==4)
	{
      ...
      hAbs_data_simRatio_alpha0_newMethod[ipt]->GetYaxis()->SetRangeUser(0.5,1.3);
   ...
   TLegend *leg= new TLegend(0.35,0.1,0.75,0.35);//0.7,0.9,0.89,0.99);
  leg->AddEntry(hdata_simRatio_alpha0[0]," 50.0 < p_{T}^{avg}(GeV/C) < 90.0 ","pl");
  leg->AddEntry(hdata_simRatio_alpha0[1]," 90.0 < p_{T}^{avg}(GeV/C) < 120.0 ","pl");
  leg->AddEntry(hdata_simRatio_alpha0[2]," 120.0 < p_{T}^{avg}(GeV/C) < 160.0 ","pl");
  leg->AddEntry(hdata_simRatio_alpha0[3]," 160.0 < p_{T}^{avg}(GeV/C) < 200.0 ","pl");
  leg->AddEntry(hdata_simRatio_alpha0[4]," 200.0 < p_{T}^{avg}(GeV/C) < 500.0 ","pl");
 ...
 //cRR->SaveAs(Form("histPlots/MC_Data_Ratio_@_\u03B1_0_VS_\u03B7.pdf"),"RECREATE");
  ...
  leg->Draw();
 cRR->SaveAs(Form("histPlots/with_without_kFSR_MC_Data_Ratio_@_\u03B1_0_VS_\u03B7.pdf"),"RECREATE");
  ...
  gPad->SetGridx();
  gPad->SetGridy();
  TLegend *leg2= new TLegend(0.35,0.1,0.75,0.35);//0.7,0.9,0.89,0.99);
  leg2->AddEntry(hdata_simRatio_alpha0[0]," 50.0 < p_{T}^{avg}(GeV/C) < 90.0 ","pl");
  leg2->AddEntry(hdata_simRatio_alpha0[1]," 90.0 < p_{T}^{avg}(GeV/C) < 120.0 ","pl");
  leg2->AddEntry(hdata_simRatio_alpha0[2]," 120.0 < p_{T}^{avg}(GeV/C) < 160.0 ","pl");
  leg2->AddEntry(hdata_simRatio_alpha0[3]," 160.0 < p_{T}^{avg}(GeV/C) < 200.0 ","pl");
  leg2->AddEntry(hdata_simRatio_alpha0[4]," 200.0 < p_{T}^{avg}(GeV/C) < 500.0 ","pl");
  ...
  gPad->SetGridy();
  leg2->Draw();
 cR2->SaveAs(Form("histPlots/absoulte_with_without_kFSR_MC_Data_Ratio_@_\u03B1_0_VS_Absoulte_\u03B7.pdf"),"RECREATE");
  ...
  leg2->Draw();
 cRA->SaveAs(Form("histPlots/Ratio_absoulte_with_without_kFSR_MC_Data_Ratio_@_\u03B1_0_VS_absloute_\u03B7.pdf"),"RECREATE");
  ...
  cPP->Divide(6,4);
    ...
    for(int ita=0;ita<nEta;ita++){
      hdata_simRatio_alpha0_Vs_Pt_perEta[ita]= new TH1F(Form("hdata_simRatio_alpha0_Vs_Pt_perEta_%.1f_%.1f",etaBin[ita],etaBin[ita+1]),Form("hdata_simRatio_alpha0_Vs_Pt_perEta_%.1f_%.1f",etaBin[ita],etaBin[ita+1]),npt,avgPtBin2);
        for( int ipt=0; ipt<npt;ipt++){
	  ...
	  hdata_simRatio_alpha0_Vs_Pt_perEta[ita]->SetTitle(Form("[%.1f < #eta < %.1f];p_{T}^{avg};<MC/Data>_{#alpha #rightarrow 0}", etaBin[ita],etaBin[ita+1]));
	}
    }
    for(int ita=0;ita<nEta;ita++){
      cPP->cd(ita+1);
      
      fit01[ita]=new TF1(Form("fit01%d",ita),fitfunct,50,500,3);
      ...
      //cPP->SaveAs(Form("histPlots/MC_Data_Ratio_@_\u03B1_0_VS_pT_per_\u03B7.pdf"),"RECREATE");
      ...
      else
	{
      ...
      /* 	     << 5 <<'\t'<< 50 <<'\t' */
      /* 	     << 500 <<'\t' */
      /* 	     << fit01[ita]->GetParameter(0) <<'\t' */
      /* 	     << fit01[ita]->GetParameter(1) <<'\t' */
      /* 	     << fit01[ita]->GetParameter(2) <<'\n'; */
    }
 ...
 // Plot graph of Ma/Data@ alpha=0 vs Pt for each ita bins//
  ...
  cPPf->Divide(6,4);
    ...
    for(int ita=0;ita<nEta;ita++){
      hdata_simRatio_alpha0_Vs_Pt_perEta_cfact[ita]= new TH1F(Form("hdata_simRatio_alpha0_Vs_Pt_perEta_cfact_%.1f_%.1f",etaBin[ita],etaBin[ita+1]),Form("hdata_simRatio_alpha0_Vs_Pt_perEta_cfact_%.1f_%.1f",etaBin[ita],etaBin[ita+1]),npt,avgPtBin2);
        for( int ipt=0; ipt<npt;ipt++){
	  ...
	  hdata_simRatio_alpha0_Vs_Pt_perEta_cfact[ita]->SetTitle(Form("[%.1f < #eta < %.1f];p_{T}^{avg};<MC/Data>_{#alpha #rightarrow 0}", etaBin[ita],etaBin[ita+1]));
	}
    }
    for(int ita=0;ita<nEta;ita++){
      cPPf->cd(ita+1);
      
      fit01_cfact[ita]=new TF1(Form("fit01_cfact%d",ita),fitfunct,50,500,3);
	...
	{
	  hdata_simRatio_alpha0_Vs_Pt_perEta_cfact[ita]->GetYaxis()->SetRangeUser(0.75,1.25);
	}

      cPPf->SaveAs(Form("histPlots/MC_Data_Ratio_@_\u03B1_0_VS_pT_per_\u03B7_newMethod.pdf"),"RECREATE");
//cout<< etaBin2[ita] <<'\t'<< etaBin2[ita+1] <<'\t'<< 5 <<'\t'<< 50 <<'\t'<< 500 <<'\t'<< fit01[ita]->GetParameter(0) <<'\t'<< fit01[ita]->GetParameter(1) <<'\t'<< fit01[ita]->GetParameter(2) <<endl;
      outFile<< etaBin2[ita] <<'\t'
	     << etaBin2[ita+1] <<'\t'
	     << 5 <<'\t'<< 50 <<'\t'
